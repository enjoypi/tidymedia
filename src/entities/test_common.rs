...
/// PNG 1.5+ `eXIf` chunk 自解析 fixture：`tests/fixtures/gen_png_exif.ts` 生成。
...
/// Panasonic RW2 RAW（TIFF 变体 magic `0x0055`）自解析 fixture：
/// `tests/fixtures/gen_rw2.ts` 生成。`IFD0` 含 Make=Canon, Model=EOS 7D,
/// `ExifIFDPointer`；`ExifIFD` 含 `DateTimeOriginal`/`CreateDate`/`ModifyDate`
/// （2017-02-14 10:30:00..02）。
pub const DATA_RW2: &str = "tests/data/sample-rw2.rw2";
/// JPEG APP1 fallback fixture：`tests/fixtures/gen_jpeg_makernotes_broken.ts` 生成。
