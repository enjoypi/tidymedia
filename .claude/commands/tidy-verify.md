...
脚本入口会自动清理 `/tmp/tm/` 中上一轮全流程产物（`run.log`/`copy_lines.log`/`exif.tsv`/`exif.err`/`bucket_result.txt`/`name_result.txt`/`run_real.log`），避免旧 source 的残留数据被 Step 2–4 误读；只删已知产物文件，不动目录里其他临时脚本。
