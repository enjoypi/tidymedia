...
/// `MediaTimeDecision` 包含 utc / offset / priority / source / `inferred_offset`
...
/// confidence 字段反映置信度（High / Low），用于"是否需要人工复核"。
#[test]
...
/// conflicts 可以包含多条（GPS + filename 同时冲突）。
#[test]
...
/// `other_source` 字段标记冲突来源（用于人工复核理解上下文）。
#[test]
...
/// GPS 冲突无 internal Source（GPS 不在 P0-P4 体系内）。
#[test]
