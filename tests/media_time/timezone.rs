...
/// EXIF 无 `OffsetTime` 时按调用方默认时区解释，标 inferred。
#[test]
...
/// 调用方可显式指定原生时区（如 P0 EXIF `OffsetTimeOriginal`）。
#[test]
...
/// decision 透传 best 候选的 offset 与 inferred 标记。
#[test]
