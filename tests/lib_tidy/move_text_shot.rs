...
//!
//! 覆盖维度：
//! - feature off：`tidy(Commands::MoveTextShot {..})` 直接返 Unsupported（`#[cfg(not)]` gate）
//! - feature off：`run_cli(["tidymedia","move-text-shot",...])` 字符串形态映射 + Unsupported
//! - feature on（model path 空）：`dispatch_move_text_shot` 在 `build_detector` 返 `InvalidInput`
//! - feature on（model path 指向不存在文件）：`build_detector` 返 Ok（懒加载），walk 到首张
//!   image 时触发 `load_runnable` Err（`_real.rs` 排除文件）→ `report.failed` 累加
//! - feature on + `--report` 写盘：dispatch 的 `if let Some(report_path)` Some 侧
//!
...
// ─────────────── feature off：Unsupported 拒绝 ───────────────

#[cfg(not(feature = "ocr-detect"))]
#[test]
fn tidy_move_text_shot_returns_unsupported_without_feature() {
    ...
    assert!(
        msg.contains("ocr-detect"),
        "expected feature-name hint, got: {msg}"
    );
}

#[cfg(not(feature = "ocr-detect"))]
#[test]
fn run_cli_move_text_shot_string_form_unsupported_without_feature() {
    let src = tempdir().unwrap();
    let out = tempdir().unwrap();
    let err = run_cli([
        "tidymedia",
    ...
    .unwrap_err();
    let msg = err.to_string();
    assert!(msg.contains("ocr-detect"), "got: {msg}");
}

// ─────────────── feature on：dispatch 路径分支 ───────────────
...
mod with_feature {
    ...
    /// 把 OCR 配置临时切到指定 model path：feature on 时 `dispatch_move_text_shot`
    /// 读全局 config，这里通过 env var + `TIDYMEDIA_CONFIG` 切独立 config.yaml。
    ...
    fn dispatch_returns_invalid_input_when_model_path_empty() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("det_model_path is empty"),
            "got: {msg}"
        );
    }
    ...
    fn tidy_returns_err_when_move_text_shot_partial_failure() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("move-text-shot partial failure"),
            "got: {msg}"
        );
    }
}
