...
- **AVI（RIFF）nom-exif 不支持**：老相机（Fujifilm FinePix 等）拍摄时间在 `LIST hdrl > LIST strl > strd` chunk（`AVIF` 魔数 + 裸 TIFF IFD，offset 基准 = strd+8），`entities/riff.rs` 自解析填 `Exif` 的 DTO/CreateDate/make/model；`entities/exif/types.rs::from_reader` 按 `video/x-msvideo` 先于泛 video 分流
...
- **覆盖率门槛 region/line/branch 三项 MUST 全 100%**（严格口径 + `--branch`）。**Windows 默认 feature 口径有既有基线 miss**（`uri.rs`/`sidecar.rs`/`filename.rs`/`resolve.rs`，多为 feature-gated + multi-binary instance）：本机判定 = 新增代码 100% + TOTAL miss 与 main 持平；怀疑回归先 `git stash` 对照
- 严格 100%：`RUSTFLAGS="--cfg=coverage_nightly" cargo +nightly llvm-cov --release nextest --summary-only --branch`；`lib.rs` / `bin/tidymedia.rs` 顶 `#![cfg_attr(coverage_nightly, feature(coverage_attribute))]`；`Cargo.toml [lints.rust] unexpected_cfgs` 注册 `cfg(coverage_nightly)`；难触发分支用 `#[cfg_attr(coverage_nightly, coverage(off))]`
- **`--branch` multi-binary instance 陷阱**：lib unit + 集成 binary 各 codegen 热点 fn 副本，每副本独立 BR 计数；cargo-llvm-cov 累加 hit count，任一 binary hit 即算 line cover。解法：①重构成 `?`（算 region 不算 branch）；②`coverage(off)` 把多 instance branch 从 BR 分母剔——**反向降 line/region cover**（lib hit 也算 0），仅用于维持 branch 100%
...
- **`--all-features` 严格覆盖也须 100%**：feature 启用侧用 `#[cfg(feature="...")]` 镜像 `tidy_rejects_*`（`tests/lib_tidy/real_factory.rs`）。`RealMtpClient::new` 是 stub 必 Err → 确定性触发 `dispatch.rs` 各 `?` Err arm；`PavaoClient::new`/`ADBServerDevice::new` 仅初始化不连网可直接断言。`factory.rs::unsupported_backend` 三 feature 全开 dead → `cfg_attr(all(coverage_nightly, 三feature), coverage(off))`。`mobile.rs` 不可达防御抽纯 helper（`expect_copy`/`expect_find`/`copy_status`/`stats_from`/`mobile_report_from`）直测；调用点用 `.map` 替代 `?` 消除永不触发的 Err region
...
- **逻辑不可达的 `?` 死区消除**：循环边界用 `while let Some(x) = buf.get(start..end)` 替手写哨兵（参考 `entities/riff.rs`）；`buf.get(idx..)?` 当 idx 来自 `position()`/`find()` 时改 `&buf[idx..]`（`?` Err arm 不可达，参考 `entities/xmp.rs::find_xmp_packet`）；纯类型安抚用 `unwrap_or(常量)` 替 `.ok()?`
