...
- 严格 100% 命令：
  - `RUSTFLAGS="--cfg=coverage_nightly" cargo +nightly llvm-cov --release nextest --summary-only --branch --ignore-filename-regex='(adapters/backend/[a-z]+_real\.rs|adapters/(ocr|face)/tract_[a-z]+\.rs)$' --all-features`
...
- **`adapters/backend/*_real.rs` 走 `--ignore-filename-regex` 而非 `coverage(off)`**：真实 client 适配器调真实 SMB/adb-server CI 不可触发，且文件规模大（每个 200+ 行）逐 fn 加 `coverage(off)` 侵入；命令行 `--ignore-filename-regex='adapters/backend/[a-z]+_real\.rs'` 排除整文件（含 `smb_real.rs`/`adb_real.rs`/`mtp_real.rs`/`factory_real.rs`），源码零 attribute。`factory.rs` 的 cfg-gated 真实装配（`build_smb_backend` 等）抽到 `adapters/backend/factory_real.rs` 符合命名约定，由同 regex 排除；Unsupported feature-off 分支保留在 `factory.rs` 由 `tidy_rejects_*` e2e 100% 覆盖
- **`adapters/(ocr|face)/tract_*_real.rs` 走 `#[coverage(off)]` 而非 ignore-regex**（与 backend `_real.rs` 相反）：文件规模小（每个 20-30 行，仅一 `load_runnable`），加单条 attribute 比维护 regex 精确排除更简。`tract_scrfd_real.rs` 因含 `decode_outputs`/`nms`/`iou` 3 fn 全 `coverage(off)`，`load_runnable` 沿用同套路。ignore-regex 只留 backend `_real.rs`（大 + 需真环境）与 tract 主体（subprocess phantom）
...
- **新增 OCR detector / TextDetector trait 方法** → `usecases/ocr/mod.rs::TextDetector` 定义（Output Port 在 use case 层）+ `adapters/ocr/fake.rs::FakeTextDetector` 同步 + `adapters/ocr/tract_dbnet*.rs` 真实实现（`tract_dbnet_real.rs::load_runnable` 加 `#[coverage(off)]`；`tract_dbnet.rs` 因含 subprocess instance phantom miss 走 ignore-regex 整文件排除——见「测试与覆盖率」节）
- **新增 Face detector / `FaceDetector`/`FaceEmbedder`/`FaceMeshDetector`/`EyeStateClassifier` trait 方法** → `usecases/face/mod.rs` 4 trait + `FaceDetection` DTO 定义（Output Port 在 use case 层）+ `adapters/face/fake.rs` 4 Fake 同步 + 4 套 `tract_{scrfd,mobilefacenet,facemesh,eyestate}.rs(+_real.rs)` 真实实现（`tract_*_real.rs::load_runnable` 加 `#[coverage(off)]`；若主体 `tract_*.rs` 出现 subprocess phantom miss 同 `tract_dbnet.rs` 一并加入 ignore-regex）+ `lib.rs` re-export 4 trait + 4 Fake（`#[doc(hidden)]`）
...
- **新增 `Backend` 能力查询 MUST 走 trait method 不硬编码 scheme 字符串**：如 `fn supports_native_rename_to(&self, other: &dyn Backend) -> bool { false }` default，`LocalBackend` override 判 `other.scheme() == "local"`；ops.rs / cull group_writer 分支 `if src.supports_native_rename_to(dst)` 代替 `if scheme=="local" && scheme=="local"`。硬编码字符串阻塞未来 SMB2 `SET_INFO` / adb `shell mv` / libmtp `SendObjectPropList` Rename 等原生 op 接入
- **新增 tracing 结构化日志 `feature` 常量 MUST 从 `usecases::report` 单点 `use` 不再本地 `const`**：`FEATURE_COPY` / `FEATURE_MOVE` + `feature_of(remove: bool)` 由 `usecases/report.rs` 单点提供；三处独立定义（copy/run.rs / archive_template.rs / adapters/report_sink.rs）任一改字面另两处漂移让日志维度分裂
- **`do_copy` 类主循环 dry_run 分支 MUST `output_index.add(src.cloned_at(target, out_be))` + 前 `src.secure_hash()?` pre-populate**：否则同 basename+同月+不同 hash 源被静默分派到同一 target 让 dry-run 报表与真跑 `_N` 后缀分裂，tidy-verify 桶对账漏 collision；pre-populate 让同 hash 后续 src 的 `exists(secure=true)` 命中 lazy cache 不 fall through 到 `open_read(target)`（dry-run 下 target 未真写会 `NotFound`）
...
- **`serde_json::to_vec_pretty` 对 f32 NaN/Inf 输出 `null` 而非 Err**（serde_json 1.0.150 实测，`{"score": null}`；早期 CLAUDE.md 曾误记「返 Err」是错的）：含 `ScoreBreakdown`/`f32` 字段的 manifest/report 生产路径**不需要** `match` Err arm，直接 `.expect("internal: manifest fields are standard types with infallible Serialize")`（rust-p0 §13 允许「已证不可达」内部错误）。同 `serde_json::to_writer_pretty` / `to_string_pretty` / `to_vec`。NaN 上游污染表现为 JSON 出现 `null` 字段（不影响下游 tidy-verify），不是 io 错误无需 best-effort skip。仅当 Manifest 含**自定义 `Serialize`** 或 **map key 非 string** 才需要 match Err arm（此时 fixture 直触发 100% 覆盖）
...
- **超大常量上限守卫 MUST 抽独立 pure helper 供 test**：如 `MAX_REMOTE_WRITE_BUFFER = 2 GiB` 让 test 无法真分配足够 buffer 触发；抽 `fn check_buffer_size(current: u64, incoming: u64) -> io::Result<()>` module-private 让 test 传假 `u64` 参数直触各 arm（`u64::MAX + u64::MAX` 走 saturating_add 亦无溢出）。同样适用「上限比对」类：`MAX_IMAGE_BYTES` / `MAX_XXX_LENGTH` / rate-limit 等（参考 `adapters/backend/remote.rs::check_buffer_size`）
- **best-effort 清理型 `if let Err(re) = X { warn }` 双 arm 抽 helper 加 `coverage(off)`**：X 是 stream_copy 内部已清理过的 remove_file，caller 层再调必 Err 走 True arm，Ok arm **逻辑不可达**（stream_copy 早清 / native early-Err 也不 create dst → remove 恒 NotFound）；helper `fn best_effort_remove_partial_dst(be, loc)` `coverage(off)` 让 False sub-branch 集中排除。参考 `usecases/cull/group_writer.rs::best_effort_remove_partial_dst`
- **`OffsetDateTime::from(SystemTime)` 内含 `.expect("Duration doesn't fit into i64::MAX")` panic**：clippy `unnecessary_fallible_conversions` 报 `TryFrom` 是 blanket infallible 误导；生产 MUST `duration_since(UNIX_EPOCH)?` + `i64::try_from` + `OffsetDateTime::from_unix_timestamp` 三段守护兜底 `OffsetDateTime::UNIX_EPOCH`，与 `epoch_to_candidate` 三段守护同哲学。P0 §2「MUST NOT 因用户输入而 panic」；参考 `usecases/copy/naming.rs::system_time_to_offsetdatetime`
- **跨 backend helper（`stream_copy` 类）判 same-scheme 优化 MUST 靠 `Arc::ptr_eq` 判同实例，不能只 `scheme() == scheme()`**：`FakeBackend("local")` vs `LocalBackend` 都声明 `"local"` 但存储互不可见，走 `src.copy_file(src_loc, dst_loc)` 得 `NotFound`；helper 若签名接 `&dyn Backend` 拿不到 Arc → caller 传 `prefer_native_copy: bool` 让 Arc 层用 `Arc::ptr_eq` 决策（参考 `entities::backend::stream_copy`；cull `copy_file_cross_scheme` 传 `Arc::ptr_eq(&src_backend, &dst_backend)`，copy `ops::stream_copy` 传 `false`）
- **`Info::cloned_at` 后 lazy_snapshot 的 `secure_hash` 若未算，`output_index.exists(secure=true)` 会触发 `f.secure_hash()` → `output_backend.open_read(target_loc)`**：dry-run 或 rename 半态下 target 未真写 → `NotFound` → `do_copy` 假失败；MUST 在 `_ = output_index.add(src.cloned_at(target, out_be))` 之前调 `src.secure_hash()?` pre-populate lazy 让快照携带 SHA-512
- **`canonical_prefix` vs `file_info::full_path` 语义正交，MUST 独立实现不共用**：前者「解析 symlink 做真实物理位置比较」→ `std::fs::canonicalize` + `strip_windows_unc`；后者「路径索引 key 希望字面稳定」→ 对 `is_absolute()` 跳过 canonicalize 避 Windows 用户目录跨盘（`C:\Users\x → D:\Users\x`）；共用让 symlink output 重叠检查静默失效
