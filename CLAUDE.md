...
- **平台条件常量优先于 `if cfg!(...)`**：跨平台行尾 / 路径分隔符等 MUST 写 `#[cfg(target_os = "windows")] pub(crate) const X: &str = "\r"; #[cfg(not(...))] pub(crate) const X: &str = "";` 而非 `if cfg!(target_os = "windows") { "\r" } else { "" }` 表达式。前者编译期常量无 dead-arm 覆盖率噪声，测试可 `use super::X` 当断言模板复用平台真值（参考 `find.rs::SCRIPT_LINE_TAIL`）
