...
按「拍摄时间」去重并整理照片/视频的多后端 CLI：扫描 sources（local/smb/adb/mtp 可混合）→ SHA-512 去重 → 按解析出的拍摄时间归档到 `output/年/月`。核心算法 = 拍摄时间判定（P0–P4 优先级，见下文「核心算法」节）。Clean Architecture 四层 + Android app（feature `android-app`）。代码注释里的 `docs/media-time-detection.md §X` 是历史 spec 引用（文件已不存在）。
...
- `bin/exiftool/exiftool.exe`（仅开发调试，非运行时依赖）：排查/修复 EXIF 时间，如 `-time:all -s <file>` 查全部时间字段、`-P '-AllDates<Filename' -overwrite_original <dir>` 按文件名批量改时间且保 mtime
  - **中文路径**：本机 portable exiftool 缺 `Win32API::File`，命令行直接传 UTF-8 路径会 `Invalid filename encoding` + 0 文件，`-charset FileName=GBK` 也告警
    - 省掉 `-charset` 让 exiftool 走 ANSI 系统调用反而最稳（stdout 显示乱码不影响 EXIF 抽取）；要 UTF-8 路径必须仿早期 tidymedia（7ea18d7 前）走 **tempfile + `-@ <file>` + `-charset filename=UTF8`**（小写 `filename`，路径写文件而非命令行）
    - 常用写入：`"-AllDates+=Y:M:D H:M:S"` 日期平移；`"-FileModifyDate<DateTimeOriginal"` mtime 同步 EXIF（对 AVI 等不可写格式也有效）；单文件按值写 `-P -overwrite_original "-AllDates=YYYY:MM:DD HH:MM:SS" "-FileModifyDate=..."` 让 tidymedia 走 P0
...
- **老 QuickTime `pnot`/`mdat` 起头 MOV**（NIKON COOLPIX S5/P5000、Casio 等 2003-2008 早期机型，及无任何头 atom、`moov` 在文件末尾的 mdat-first 变体）nom-exif 3.6.0 `parse_bmff_mime` 拒识：只认 `ftyp`/`wide` 首 box，`HEADER_PARSE_BUF_SIZE=128B` 也扫不到 `mdat` 兜底
  - 当前 fork patch 在入口短路（首 box header==pnot|mdat 直返 QuickTime mime；**必须 `BoxHeader::parse` 仅解析 header**——`BoxHolder::parse` 要求整 box body 在 buffer 内，数 MB 的 mdat 必 Incomplete 让短路静默失效），`Cargo.toml [patch.crates-io] nom-exif = { git = "https://github.com/enjoypi/nom-exif.git", branch = "feat/legacy-quicktime-pnot" }`
  - 后续 `mov.rs::extract_moov_body_from_buf` 经 `ClearAndSkip` 跳过任意大 mdat 解析 `mvhd.creation_time`，与 ExifTool 同源，无需自写 quicktime.rs
  - fork 本地 clone：`D:/prj/nom-exif-git`（无 `.cargo/config.toml`，构建前从 tidymedia 复制 MSVC 链接配置，已加 `.git/info/exclude`）；上游 PR 合并发版后切回 `*` 删 patch
...
- **AVI（RIFF）nom-exif 不支持**：老相机（Fujifilm `FinePix` 等）AVI 的拍摄时间在 `LIST hdrl > LIST strl > strd` chunk（`AVIF` 魔数 + 裸 TIFF IFD，offset 基准 = strd+8），由 `entities/riff.rs` 自解析填入 `Exif` 的 `date_time_original`/`create_date`/`make`/`model`（P0/P1 候选 + `{make}/{model}` 复用既有链路，无新增 `Source`）；`entities/exif/types.rs::from_reader` 按 `video/x-msvideo` 先于泛 video 分流
- **M2TS/AVCHD（BDAV MPEG-TS）nom-exif 不支持**：Canon 等摄像机拍摄时间在 H.264 SEI `user_data_unregistered`（MDPM UUID，exiftool 读作 `H264:DateTimeOriginal`），由 `entities/m2ts.rs` 自解析——按 AVCHD 固定 video PID 0x1011 重组 192B BDAV 包 payload 后直搜 UUID（不解析 PMT/NAL，YAGNI；PID 重组天然解决 UUID 跨包切断），EP 字节（`00 00 03`）按 RBSP 规范剥离（午夜 00:00:00 BCD 会触发）；MDPM 布局已对真实文件逐字节对账 exiftool：tag `0x18`=[时区,世纪,年,月] BCD、`0x19`=[日,时,分,秒] BCD，时区字节忽略走 naive+配置时区口径；仅填 `date_time_original`（单一拍摄时刻不伪造 P1）；`entities/exif/types.rs::from_reader` 按 `video/m2ts` 先于泛 video 分流；fixture `tests/data/sample-canon-avchd.m2ts`（真实 Canon 截前 1024B）
- **JPEG/HEIC/TIFF XMP packet 单段 fallback**：相机原拍后被 Lightroom/Bridge/ACR re-tag 的图，EXIF IFD0 可能仅剩 `ModifyDate`，原始拍摄时间落在 XMP `<x:xmpmeta>` packet 的 `photoshop:DateCreated`（→ DTO/P0）与 `xmp:CreateDate`（→ CreateDate/P1）
  - `populate_image_dates` 在 EXIF DTO+CreateDate 双 0 时扫已 buffer 头部 64 KB（`XMP_SCAN_BYTES`），由 `entities/xmp.rs` 抽 attribute 形式（单/双引号都支持，element 形式 YAGNI、ExtendedXMP YAGNI）；`adapters/sidecar.rs::parse_xmp_date` 复用同实现避免两份漂移
  - fixture 用 `bin/exiftool -api Compact="Shorthand,NoPadding"` 才出 attribute 形式（默认 element）
...
- **`--branch` multi-binary instance 陷阱**：lib unit + 集成 binary 各自 codegen 热点 fn 副本，每副本独立 BR 计数器；某 binary 未触发即报 instance miss。**cargo-llvm-cov merge 累加 hit count（不取 min）**，任意 binary hit 即算 line cover。可行：①重构成 `?`（算 region 不算 branch）；②函数级 `coverage(off)`——**用途是把多 instance branch 从 BR 分母剔除维持 branch 100%，不是提升 line/region 覆盖率**；cov(off) 让该 fn 在**所有** binary 都不 instrument，**反向降低** line/region cover（lib hit 也算 0）。全删 hot fn cov(off) 会让 branch 立刻出 ~9 miss、line miss 反增 ~13（noise 仍在）
...
- **chmod 触发 stat 类 IO 错误**：chmod 0 **文件** stat 仍成功（要 parent dir execute 位）；chmod 0 **parent dir** 才让 `Path::try_exists` / `fs::metadata` 返 `PermissionDenied`。已有 chmod 555 src 父目录 cover rename 场景，stat / exists Err 传播测试同套路（参考 `local_edge_tests.rs::exists_propagates_permission_denied_from_unreadable_parent`）
- **测试 env::{set_var,remove_var} 收敛 helper 套路**（P0 §13 SAFETY 注释 DRY）：测试模块顶 `#![allow(unsafe_code)]` + 单点 `fn set_env_var(name: &str, value: &str) { /* SAFETY: nextest 每测试独立进程 */ unsafe { std::env::set_var(name, value) }; }` / `remove_env_var` 两个 helper，让 SAFETY 注释只需一份（参考 `frameworks/config.rs::tests`）。**sed 批量替换陷阱**：`sed -E 's/unsafe \{ std::env::set_var\((.*)\) \};/set_env_var(\1);/g'` 会把刚定义的 helper 自身实现也匹配掉致无限递归 → 先 sed 后手动改回 helper 内 unsafe 块
- **平台条件常量优先于 `if cfg!(...)`**：跨平台行尾 / 路径分隔符等 MUST 写 `#[cfg(target_os = "windows")] pub(crate) const X: &str = "\r"; #[cfg(not(...))] pub(crate) const X: &str = "";` 而非 `if cfg!(target_os = "windows") { "\r" } else { "" }`。`cfg!()` 让 LLVM instrument 两个 arm、dead arm 必 miss（实测 region +11/line +3 miss）；双 `#[cfg]` 让 cfg-out 平台代码不编译，**仅 lcov DA 报 0 noise**（不影响 region/branch counter）。**业务逻辑跨平台分裂应消除而非容忍**——如 find 删除脚本曾分 sh/cmd 双轨已统一 Python（消除 3 处 cfg）
...
- **`cargo +nightly llvm-cov report` 子命令不接 `--all-features`**：`error: invalid option '--all-features' for subcommand 'report'`。仅 `nextest` 子命令接受 feature flag——先 `... llvm-cov ... nextest --all-features` 跑，再 `... llvm-cov report --release --text`（复用 profdata、feature 集随之）
...
- **大文件拆 → 主题切多份 tests 外置**：生产 + tests ≥ 400 行时按职责切主题文件。tests 共享 helper 抽 `<mod>_tests_common.rs`（`pub(super) fn helper`），其他主题用 `use super::tests_common::helper`；主 mod 顶层挂 `#[cfg(test)] #[path = "<mod>_xxx_tests.rs"] mod xxx_tests;`（**勿嵌套到 `mod tests {}` 内**——`#[path]` 在嵌套 mod 下基于父 mod 目录解析，外置文件会找不到）。子 mod 内跨模块引用其它 entities 项用 `crate::entities::...` 绝对路径而非 `super::super::...`（语义更清晰）。生产文件目录化时 struct 字段改 `pub(super)` 让 sibling sub-mod 直接构造字面量；mod.rs 用 `#[cfg(test)] use self::sub::item;` 给 tests 子 mod 私有 re-export（参考 `entities/exif/mod.rs` 6 主题 tests + tests_common 模式、`entities/media_time/resolve_*tests.rs`、`entities/riff_*tests.rs`、`frameworks/config_*tests.rs`）
...
- **新增 `Backend` impl** → `walk` MUST 递归 yield 所有 file 条目：LocalBackend 走 `WalkBuilder`、RemoteBackend 用 `walk_recursive` 自驱 `EntryKind::Dir` 子树；单层 list 会让 `Index::visit_location` 在 `kind != File` 过滤 Dir 永不下钻，远端 source 子目录文件整子树静默丢失
...
- **QuickTime/视频容器时间是 UTC，对账须加时区——「前 7 字符」规则仅对 EXIF naive 成立**：`QuickTime:CreateDate`(mvhd) / `QuickTime:CreationDate`(`com.apple.quicktime.creationdate`) 按 QuickTime spec 是 UTC
  - nom-exif 合成带时区 `DateTime`，tidymedia `entry_value_to_epoch` 走 `dt.timestamp()`（UTC epoch）分支再 `.to_offset(+tz)` 归档，与 EXIF naive 的 `+offset` 解释再 `+offset` 归档（首尾抵消）**口径不同**——归档桶比 UTC 字符串晚 tz 小时，月末/午夜会跨月
  - exiftool 默认以 raw UTC 显示 QuickTime 时间且**不加时区后缀**（`-api QuickTimeUTC=0` 行为），故 `tidy-verify` 脚本 `compare_buckets.py` 对 QT 列（`QT_COL_INDICES=(2,3)`）必须 `_qt_bucket()` 按 UTC→配置时区转换后取年月（带 `±HH:MM`/`Z` 后缀者按其 offset 归一化）；否则月末 video 被误判为 `TidymediaContainerMiss`（实为 tidymedia 已读对容器时间、归档正确，是脚本时区盲区）
  - `03_compare_buckets.sh` 从 env > `config.yaml` 的 `timezone_offset_hours:${...:-N}` > 8 解析时区传第 3 参数
...
- **冲突 `diff_secs` 约定** = `chosen.utc - other_utc`（`detect_conflicts` 各分支统一）；Override 分支 chosen=winner、other=原 P0 best，新增 `ConflictKind` 按此口径，写反让 tidy-verify 等下游判相机时钟偏差方向颠倒
- **`EPOCH_1904` 过滤**用 `[EPOCH_1904, EPOCH_1904+86400)` 整天窗口而非精确等值——少数编码器把 `mvhd.creation_time` 写 1/2/... 让 ts=EPOCH_1904+1 漏过精确匹配落进 1904/01 桶
...
- 错误文案映射：SMB / ADB `map_error` MUST 同时支持 **`NotFound`**（`enoent` / `no such file` / `does not exist`）+ **`PermissionDenied`**（`eacces` / `permission`）两类 ASCII 文案重映射，否则真实远端的 `io::Error::other("pavao: ENOENT ...")` 让 `mkdir_recursive` 的 `NotFound` guard 永不触发 → 多层归档目录创建必败（SMB 漏 ENOENT 文案曾数月未被发现）。**Fake 干净 `ErrorKind::NotFound` 会掩盖该缺陷**——专测 MUST 用 `FakeRemoteClient::with_error_factory(|k| match k { NotFound => Error::other("pavao: ENOENT ...") })` 模拟生产文案；文案匹配 MUST 用 `to_ascii_lowercase()` 而非 `to_lowercase()`（避免 Unicode full-case folding 在本地化设备消息上字节漂移）
...
- **R1 外置**：`copy.{timezone_offset_hours, unique_name_max_attempts, archive_template}` / `exif.valid_date_time_secs` / `backend.smb.{default_user,workgroup}` / `backend.adb.{server_host,server_port}` / `log.level`（CLI `--log-level` 缺省值，优先级 `RUST_LOG` > flag > 配置）。**无消费点的字段勿先加占位**——库/实现就绪时随消费链一起加
...
- **存在性查询的 Err MUST 传播，MUST NOT `unwrap_or(false)`**：把"查询失败"吞成"不存在"会让后续 open_write truncate 覆盖已有文件、move 模式连源一起删（naming.rs 已踩坑修复）；保守兜底方向是 `true`（多重试）而非 `false`（敢覆盖）。**`std::path::Path::exists` / `camino::Utf8Path::exists` 返 `bool`、内部吞 `PermissionDenied` 等 IO 错误成 false 是同一类陷阱**——`LocalBackend::exists` MUST 用 `path.as_std_path().try_exists()?` 让 Err 经 `?` 上抛，否则同样误判覆盖
...
- **`Info::cloned_at(loc, backend)` 复用 src hash 入 `output_index`**：copy/move 完成对 dst 重新 `Info::open` 既浪费（远端额外 RTT + 4 KiB 读）又会在 NFS ESTALE / 防病毒抢占下让 dst 已写但未入索引 → 后续同 hash 源 unique-name 再写一份成重复副本。`cloned_at` 复用 src 已算 hash/EXIF/Lazy 状态零 IO 通吃两病
- **跨设备 `fs::rename` fallback 是 copy+remove 非原子**：copy 成功 remove 失败的半态 Err MUST 文案含 `copied ... but cannot remove source`，让 `do_copy` / failed 计数与 "copy 也失败" 场景区分；否则用户误判后重跑会把已被 copy 的源也删（move 模式即丢源）
- **跨 scheme sibling 路径用 `Location::with_path`/`path` 单点**：sidecar `with_extension` / `append_suffix` 不必各 backend 各实现——远端 backend 通过 `Location` 自然等价；同套路适用任何"同目录另一文件名"的查找
- **`use rayon::prelude::*` 违反 P0 §5**：最小 import 集 = `use rayon::iter::{IntoParallelRefIterator, IntoParallelRefMutIterator, ParallelIterator}`（`par_iter_mut` 需 RefMut，少了编译失败）
...
- **find 删除脚本统一 Python 输出**：`render_script` 输出 `#!/usr/bin/env python3` shebang + `import os` 头 + `os.remove("...")` 待删 / `# os.remove("...")` 注释保护，按 `DuplicateGroup` size 降序分组。用户审查 → 取消注释 → `python3 script.py`。**路径转义**：`s.replace('\\', "\\\\").replace('"', "\\\"")`（windows 反斜杠 + 含 `"` 的极罕见路径）；含 `\n` 控制字符暂不处理（YAGNI）。**对外契约**：tidy-verify 等脚本不消费 find 输出，README 仅描述用法不锁格式；变更输出格式无破坏性影响
