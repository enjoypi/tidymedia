...
- **feature `ocr-detect`（默认 off）**：`move-text-shot` 子命令的 OCR 文本检测后端，引入 `tract-onnx`（纯 Rust ONNX 运行时，sonos 生产用、ARM/WASM 支持）+ `image`（PNG/JPEG/HEIC 解码）；模型文件**不入 git**——用户自备 `PaddleOCR DBNet det.onnx`（约 5–10 MB），通过 `backend.ocr.det_model_path` / `TIDYMEDIA_OCR_DET_MODEL` 指定。未启用 feature 时子命令仍编译，dispatch 返 `Unsupported`（与 smb/adb/mtp feature off 同套路）
...
- 严格 100%：`RUSTFLAGS="--cfg=coverage_nightly" cargo +nightly llvm-cov --release nextest --summary-only --branch --ignore-filename-regex='(_real\.rs|adapters/ocr/tract_dbnet\.rs)$' --features ocr-detect`；`lib.rs` / `bin/tidymedia.rs` 顶 `#![cfg_attr(coverage_nightly, feature(coverage_attribute))]`；`Cargo.toml [lints.rust] unexpected_cfgs` 注册 `cfg(coverage_nightly)`。`tract_dbnet.rs` 入 ignore-regex 是因 subprocess `tidymedia` bin instance（cli_smoke / run_cli_flags）链接进该文件但不调用 OCR 路径，LLVM 累加产生 1 region phantom miss——纯算法（preprocess/decide/target_size/align32）已由 lib unit 真实覆盖，行为正确性不受 ignore 影响
...
- **新增子命令（如 `move-text-shot`）** → `adapters/cli.rs::Commands` enum 加 variant + `adapters/dispatch.rs::CommandResult` 加 variant（feature-gated 子命令用 `#[cfg(feature="...")]` 三处对齐：`CommandResult` variant + `tidy_with` match arm + `tidy()` partial-failure arm）+ `usecases/<name>/` 目录 + `usecases/report.rs::Report` + `adapters/report_sink.rs` match arm + `lib.rs` re-export + e2e MUST 含 `run_cli(["tidymedia","<sub>",...])` 字符串形式；feature off 与 on 各跑一次集成测试（参考 `tests/lib_tidy/move_text_shot.rs`）
