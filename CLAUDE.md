...
- **QuickTime/视频容器时间是 UTC，对账须加时区——「前 7 字符」规则仅对 EXIF naive 成立**：`QuickTime:CreateDate`(mvhd) / `QuickTime:CreationDate`(`com.apple.quicktime.creationdate`) 按 QuickTime spec 是 UTC；nom-exif 合成带时区 `DateTime`，tidymedia `entry_value_to_epoch` 走 `dt.timestamp()`（UTC epoch）分支再 `.to_offset(+tz)` 归档，与 EXIF naive 的 `+offset` 解释再 `+offset` 归档（首尾抵消）**口径不同**——归档桶比 UTC 字符串晚 tz 小时，月末/午夜会跨月。exiftool 默认以 raw UTC 显示 QuickTime 时间且**不加时区后缀**（`-api QuickTimeUTC=0` 行为），故 `tidy-verify` 脚本 `compare_buckets.py` 对 QT 列（`QT_COL_INDICES=(2,3)`）必须 `_qt_bucket()` 按 UTC→配置时区转换后取年月（带 `±HH:MM`/`Z` 后缀者按其 offset 归一化）；否则月末 video 被误判为 `TidymediaContainerMiss`（实为 tidymedia 已读对容器时间、归档正确，是脚本时区盲区）。`03_compare_buckets.sh` 从 env > `config.yaml` 的 `timezone_offset_hours:${...:-N}` > 8 解析时区传第 3 参数
